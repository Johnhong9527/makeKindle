...
(async () => {
  ...
  const result = await page.evaluate((config) => {
    ...
    return new Promise(resolve => {
      ...
      let arr = [];
      let index = 0;
      let _dd = document.getElementById('list').getElementsByTagName('li');
      let len = _dd.length;
      Object.keys(_dd).forEach(index => {
        if (index > 0 && index < 191) {
          console.log(_dd[index].innerText);
          let _title = _dd[index].innerText;
          let _url = _dd[index].getElementsByTagName('a')[0].getAttribute('href');
          arr.push({
            href: _url,
            title: _title,
